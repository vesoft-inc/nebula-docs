...
rocksdb_disable_wal | true | 使用rocksdb的wal。默认使用raft wal | |
wal_ttl     | 14400 (秒)  | RAFT wal 保留时间 | UPDATE CONFIGS 命令修改，立刻生效 |
rocksdb_db_options  | {} | json 类型，其中每个参数 key 和 value 均为 string 格式。解释见下 | UPDATE CONFIGS 命令修改，立刻生效 |
rocksdb_column_family_options | {} |  json 类型，其中每个参数 key 和 value 均为 string 格式。解释见下 | UPDATE CONFIGS 命令修改，立刻生效 |
rocksdb_block_based_table_options | {} | json 类型，其中每个参数 key 和 value 均为 string 格式。解释见下 | UPDATE CONFIGS 命令修改，立刻生效 |
...
max_edge_returned_per_vertex    -- 解释见下文
```
