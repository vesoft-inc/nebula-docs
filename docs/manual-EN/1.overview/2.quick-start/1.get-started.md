...
- `partition_num` specifies the number of partitions in one replica. It is usually 5 times the number of the cluster hard disk.
- `replica_factor` specifies the number of replicas in the cluster. It is usually 3 in production, 1 in test.
...
Likewise, you can put hundreds or thousands DML (Data Manipulation Language) statements in a *data.ngql* file to insert data.

## Data Import Tools

If you have millions of records to insert, it is recommended to use the [csv importer](../../3.build-develop-and-administration/5.storage-service-administration/data-import/import-csv-file.md) (or [sst ingest tool](../../3.build-develop-and-administration/5.storage-service-administration/data-import/download-and-ingest-sst-file.md)).
