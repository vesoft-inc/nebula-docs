...
!!! Note
    ...
    Memory Tracker is a memory management tool designed to monitor and limit memory usage. For large-scale queries, Memory Tracker can prevent memory leaks and Out Of Memory (OOM) issues. If you're using Memory Tracker in a containerized environment, you need to add the relevant configurations to the configuration files of the Storage and Graph services.

    1. Create directories `/sys/fs/cgroup/graphd/` and `/sys/fs/cgroup/storaged/`, and configure the `memory.max` in each directory.
    2. Add the relevant configurations to `etc/nebula-graphd.conf` and `etc/nebula-storaged.conf`:
