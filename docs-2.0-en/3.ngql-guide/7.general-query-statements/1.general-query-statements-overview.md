...
This topic provides an overview of the general categories of query statements in NebulaGraph and their Use cases.
...
- Must specify the tag or edge type of the vertex or edge.
...
**Use case:** Complex graph pattern matching is required, such as finding combinations of vertices and edges that satisfy a specific pattern.
...
**Usage:** Query paths between given start and target vertices or querying properties of vertices and edges along paths.
...
FIND SHORTEST PATH FROM "player102" TO "team204" OVER * YIELD path AS p;
     ──┬─────            ───────────┬─────────── ───┬───      ────┬────
       ...
       │           │                                              
       │           └──────From the given start and target VIDs            
       │                                                   
       │    
       │                                                    
...
**Usage:** Extract a portion of the graph that satisfies specific conditions and query properties of vertices and edges in the subgraph.
...
When referencing the results of a subquery in a compound statement, you need to alias the result and use the [pipe symbol](../5.operators/4.pipe.md)(`|`) to pass it to the next subquery. Use `$-` in the next subquery to reference the alias of that result. See [Pipe Symbol](../5.operators/4.pipe.md) for details.
