...
- Install the new `rpm` package on each machine
  - Run `https://github.com/vesoft-inc/nebula/releases/tag/v{{ nebula.release }}` command to download the package
- Then run `rpm -Uvh nebula-{{ nebula.release }}.el7-5.x86_64.rpm` command to install Nebula Graph
