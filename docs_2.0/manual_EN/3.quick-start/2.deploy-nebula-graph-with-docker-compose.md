...
4. Use nebula-console to connect to Nebula Graph.
    ...
    ```bash
    $ docker run --rm -ti --network nebula-docker-compose_nebula-net vesoft/nebula-console:v2-preview-nightly --address=graphd --port=3699
    ```

    >**NOTE**: By default, the authentication is disabled. Once it is enabled, you have to use the `-u <username>` and the `-p <password>` options to specify the username and password in the above command. To enable authentication, see [Authentication Configurations](https://docs.nebula-graph.io/manual-EN/3.build-develop-and-administration/4.account-management-statements/authentication/#authentication).
...
Run the following command to list the service information of Nebula Graph, such as state and ports.
