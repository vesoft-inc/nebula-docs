...
4. Use nebula-console to connect to Nebula Graph.
    ...
    >**NOTE**: By default, the authentication is disabled. Use any username or password to connect Nebula Graph. The default username is `root` and the default password is `nebula`.
...
Run `docker-compose ps` to list the service information of Nebula Graph, such as status and port.
