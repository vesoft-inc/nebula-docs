...
* Create a graph space:
    ...
    |vid_type | To insert vertices with string type IDs, specify the fixed string length for the vertices IDs.|
...
* Use `GO`: Search for the followings of the VID "player100". Filter the followings whose age is equal to or greater than 35. Rename the columns in the result with `Teammate` and `Age`.
...
* Use `GO`: Search for the followings of the VID "player100". Then Retrieve the teams of the followings. To combine the two queries, use a pipe or a temporary variable.
