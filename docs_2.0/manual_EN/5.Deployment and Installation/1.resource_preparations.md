...
* [What do I need to run Nebula Graph in a production environment?](#requirements)
...
See [General software requirements](#general-software-requirements).
...
For now, we can only install Nebula Graph in the Linux system. To install Nebula Graph in a test environment, we recommend that you use any Linux system with kernel version 2.6.32 or above.
...
You can adjust some of the kernel parameters to better accommodate the need for running Nebula Graph. For more information, see [Optimize Linux kernel configuration](TODO).
...
You must have the correct version of the software listed below to install Nebula Graph in your system.
...
| Number of partitions for a graph space | _the number of disks in the cluster_ * _disk_performance_index_                                                                                                                |
...
* Question 2: How to get the number of RocksDB instances?
    ...
    > **NOTE**: You can decrease the memory size occupied by the bloom filter by adding `--enable_partitioned_index_filter=true` in `etc/nebula-storaged.conf`.

* Question 3: What is the _disk_performance_index_?

    Answer: _disk_performance_index_ is a value between 2 to 10, the better performance of the hard disk, the larger the value. Use 2 for HDD.

## Optimize the configuration for HDD and Gigabit Networks

Nebula Graph is intended for NVMe SSD and 10 Gigabit Networks. Optimizing the configuration as follows may better accommodate HDD and Gigabit Networks.
