...
Execution planner processes the parsed nGQL statements into actions. An action is the smallest unit that can be executed. A typical action could be fetching all neighbors of a given vertex, getting the properties of an edge, or filtering vertices or edges based on the given condition. Each action is assigned to a operator which performs the action.

For example, a simple `SHOW TAGS` statement is processed as two actions and assigned to a `Start` operator and a `ShowTags` operator, while a more complex `GO` statement may be processed as more than 10 actions and assigned to operators of the same number.
...
The return message of an `EXPLAIN` or a `PROFILE` statement has two formats, the default format "row" and format "dot". You can use the `format` option to modify the output format.
...
You can use the `format="dot"` option to output the return message in the DOT language, then use Graphviz to generate a graph of the plan. 
