...
* `VID` is the vertex ID. In Nebula Graph 2.0.0-Alpha, only string type VID is supported. The maximum VID length is set when creating the graph space. For detail information on the maximum VID length, see [Create Space document](../9.space-statements.md/1.create-space.md).
...
`prop_value_list` must provide the values in the sequence of the properties given in `prop_name_list`. If a property value does not match the data type specified in the tag, an error is returned. When the `NOT NULL` constraint is set for a given property, an error occurs if no value is given. When the default value for a property is `NULL`, you can leave it blank and no error occurs.
