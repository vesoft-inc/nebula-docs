...
|`sources.csv.delimiter`   |`,`| No |  Specifies the delimiter for the CSV file. Only 1-character string separators are supported. When using special characters as separators, they need to be represented using escape sequences and enclosed in double quotes for proper escaping. For example, the tab character `\t` should be written as `"\t"` instead of `'\t'`, and `0x03` in hexadecimal, i.e. `Ctrl+C`, should be written as `"\x03"` or `"\u0003"`. For details on escaping special characters in yaml format, see [Escaped Characters](https://yaml.org/spec/1.2.2/#escaped-characters).|         |  
