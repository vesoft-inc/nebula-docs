...
Nebula Importer (Importer) is a standalone import tool for CSV files with [NebulaGraph](https://github.com/vesoft-inc/nebula). Importer can read the local CSV file and then import the data into the NebulaGraph database.
...
1. Clone repository.
  ...
  !!! note
        ...
        NebulaGraph 1.x and 2.x have different RPC protocols, so:
