...
4. Set alert rules.
  ...
  2. On the **Condition** tab, set metric type, rule, and alert duration.
    ...
    | Metric Rule | Click **+ Add condition** to set metric rules for a node or a service. It supports add composite conditions (`AND` relationship). For more information, see [Monitoring metrics](../7.monitor-parameter.md).|
