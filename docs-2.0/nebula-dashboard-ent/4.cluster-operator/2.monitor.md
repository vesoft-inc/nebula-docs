...
Users can view the overview of the node resources, including the IP, node name, runtime, CPU cores, memory, and other information for all nodes.

### Single-node monitoring

Users can switch nodes on the right side to view the monitoring information of the specified node.
...
| ------------ | ------------------------------------------------------------ |
| Upper middle area | 1. The health degree of your cluster. The system scores the health of your cluster. For more information, see [Cluster diagnositics](../analysis-diagnosis/cluster-diagnosis.md). <br>2. The information and number of running nodes, the number of running services and abnormal services in the cluster. <br/>3. CPU and memory usage of the node at the current time.<br/>4. Alert notifications. The system displays the 5 most recently triggered alert messages based on their severity level (emergency>critical>warning). For more information, [Monitoring alerts](../4.cluster-operator/9.notification.md). |
