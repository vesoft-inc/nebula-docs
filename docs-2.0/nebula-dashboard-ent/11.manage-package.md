...
3. Click **Download Package**, select the installation package you want to download. The description are as follows:
  ...
  - Version: Supports stable versions later than v2.5.
...
3. Click **Upload Package**, select the local installation package you want to upload. View the upload progress on the upper of the page and wait until the upload is complete.
...
Because we use Nginx as the reverse proxy, the default limit for uploaded files is 1 MB. Add `client_max_body_size 200m;` to the `http{}` section of `nginx.conf`, that means the file of up to 200 MB is allowed to be uploaded.
