...
- You can filter alert messages by date and time, period. The period includes 1 hour, 6 hours, 12 hours, 1 day, 3 days, 7 days, and 14 days.
...
Alert messages cannot be deleted. In the `nebula-dashboard-ent/config/config.yaml` file, `messageStore` is for setting the number of days to save alert messages, the value of which is 90 by default.
...
4. Set alert rules.
  1. On the **Basic Information** tab, set alert name, severity, and frequency.
    ...
    | -------- | ------------------------------------------------------------ |
    | Alert Name | Set a name for an alert rule. The name can only contain lowercase letters, numbers, and separators (`-`), and must begin and end with a lowercase letter or number. The name contains up to 253 characters.                                    |
  ...
  2. On the **Condition** tab, set metric type, rule, and alert duration.
    ...
    | Metric Rule | Set metric rules for a node or a service. <br>For more information, see [Monitoring metrics](7.monitor-parameter.md).|
  ...
  3. On the **Message Settings** tab, you can see the rule summary and rule details, and then click **Submit**.
    ...
    !!! note   
        ...
        It is not recommended for novice users to modify rule details, otherwise, an error may occur. 
...
In the **Custom Rules** list, select the target rule, and then click the edit icon ![](figs/alert_edit.png) to edit the rule.

### Delete custom rules

In the **Custom Rules** list, select the target rule, click the delete icon ![](figs/alert_delete.png) to delete the rule.
...
1. At the top navigation bar, Click **Cluster Management**, and on the right side of the target cluster, click **Detail**.
2. On the left-side navigation bar of the **Cluster Management** page, click **Notification**->**Recipient Setting**.
3. On the **Recipient Setting** page,
