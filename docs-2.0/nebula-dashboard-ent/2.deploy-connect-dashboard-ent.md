...
This topic will introduce how to install and deploy Dashboard Enterprise Edition in detail.
...
1. Select and download the TAR package according to your needs. It is recommended to select the latest version.
  ...
  !!! enterpriseonly
        ...
        To download the TAR package, contact sales. For features of Dashboard Enterprise Edition, see [Pricing](https://nebula-graph.com.cn/pricing/).
...
4. Copy the License file into the `nebula-dashboard-ent` directory.

  ```bash
  cp -r <license> <nebula-dashboard-ent_path>
  ```
...
5. Run the following commands to stop and restart Dashboard Enterprise Edition. 

  ```bash
  systemctl stop nebula-dashboard-ent # Stop the service.
  systemctl start nebula-dashboard-ent # Start the service.
  ```
