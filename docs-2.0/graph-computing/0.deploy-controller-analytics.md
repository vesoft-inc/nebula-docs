...
The Dag Controller service only provides scheduling capabilities and will not crash, but the Nebula Analytics service may crash due to insufficient memory when writing too much data to HDFS or Nebula Graph.

### Can I continue a job from a failed task?

Not supported. You can only re-execute the entire job.

### How to speed up if a task result saved slowly or data transferred slowly between tasks?

The Dag Controller contains graph query components and graph computing components. Graph queries send requests to graph service for single-threaded queries, so the graph queries can only be accelerated by increasing the memory of the graph service. Graph computing is performed on distributed nodes provided by the Nebula Analytics, so the graph computing can be accelerated by increasing the size of the Nebula Analytics cluster.
