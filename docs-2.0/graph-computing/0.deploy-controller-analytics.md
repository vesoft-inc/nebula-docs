...
The Dag Controller contains graph query components and graph computing components. Graph queries send requests to a graph service for single-threaded queries, so the graph queries can only be accelerated by increasing the memory of the graph service. Graph computing is performed on distributed nodes provided by Nebula Analytics, so graph computing can be accelerated by increasing the size of the Nebula Analytics cluster.
