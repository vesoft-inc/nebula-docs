...
Dag Controller is a task scheduling tool that can schedule the jobs which type is DAG (directed acyclic graph). The job consists of multiple tasks to form a directed acyclic graph, and there is a sequence relationship between the tasks.

The Dag Controller can perform complex graph computing with the Nebula Analytics. For example, the Dag Controller sends an algorithm call request to the Nebula Analytics, which saves the result to Nebula Graph or HDFS, the Dag Controller then takes the result as input to the next algorithmic task to create a new task.
...
There are some differences between installation packages and commands in different environments. The installation environment in this document is as follows:
...
- The BFS and SSSP algorithms need to verify the parameter `root`. They support only one upstream component and must specify rows and columns. If multiple files exist, a random file is selected. If row, column, or file is not found, an error will be reported.
