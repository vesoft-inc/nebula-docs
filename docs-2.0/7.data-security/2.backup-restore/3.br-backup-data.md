...
After the BR is compiled, you can back up data of some specified graph spaces. This article introduces how to use the BR to back up data.
...
- Create a diretory with the same absolute path on the meta and the storage servers for the backup files and get the absolute path. In this example, `/home/user/backup/` is used.
...
1. Edit the configuration file as follows.
    ...
    meta_nodes:
     - # Set the IP address and the port of one meta server
      ...
      user: "nebula"
     #- # If more than one metad process runa, refer the preceding configuration to add more
     #  addrs: "192.168.8.161:9559"
     #  root: "/usr/local/nebula/"
     #  data: "/usr/local/nebula/data/meta"
     #  user: "nebula"
    ...
    storage_nodes:
     - # Set the IP address and the port of one storage server
      ...
      user: "nebula"
     #- If more than one storaged process runs, refer the preceding configuration to add more
     #  addrs: "192.168.8.161:9779"
     #  root: "/usr/local/nebula/"
     #  data: "/usr/local/nebula/data/storage"
     #  user: "nebula"
...
3. Run the command to back up data.
   ...
   > **NOTE**: During the backup process, if the leader changes, an error occurs.

   When the backup is successful, you can find the backup files with names of the `BACKUP_YY_MM_DD_HH_mm_SS` format in the backup store directory. In this example, in the `/home/user/backup/` directory, you can find a directory named `BACKUP_2020_11_30_20_47_44`.
...
If you store the backup files on the meta or storage servers, you can back up them in some NAS (Network Attached Storage) for data security.
