...
- The BR is compiled. For more information, see [Compile Backup & Restore](2.compile-br.md).

- The Nebula Graph services are ONLINE.

- Make sure that the topology of the Nebula Graph cluster is the same with that of the backup files, including the number of graphd, storaged, and metad processes, and the number of graph spaces.
...
- Your account on the BR machine can log on to the meta and the storage servers via SSH without password. Here is a [configuration reference](http://alexander.holbreich.org/ssh-tunnel-without-password/ "Click and leave Nebula Graph Docs"). This account must have the write permission to the installation directory of Nebula Graph. In this example, such an account named `nebula` on the BR machine is used.
...
- If the backup files are stored in other storage systems, make sure they have been moved to the meta and storage servers.
...
1. Edit the configuration file as follows.
    ...
    meta_nodes:
      - # Set the IP address and the port of one meta server
        ...
        user: "nebula"
     #- # If more than one metad processes runs, refer the preceding configuration to add more
     #- addrs: "192.168.8.161:9559"
     #  root: "/usr/local/nebula/"
     #  data: "/usr/local/nebula/data/meta"
     #  user: "nebula"
...
3. Restore data.
   ...
   In this command:
    ...
    > **NOTE**: During the restoration process, if the leader changes, an error occurs.

    When the restoration is successful, you can find the data in the `/data/storage` directory under the Nebula Graph installation directory.

4. Verify the data. For example, run [`SHOW STATS`](../../3.ngql-guide/7.general-query-statements/6.show/14.show-stats/) to get the number of vertices and edges in the restored graph space.
