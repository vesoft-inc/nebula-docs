...
- `NebulaConnectionConfig` is the configuration for connecting to the NebulaGraph, as described below.
  ...
  |`withConnectionRetry`  |No| The number of retries that the Nebula Java Client connected to the NebulaGraph. The default value is `1`.  |
...
- `NebulaConnectionConfig` is the configuration for connecting to the NebulaGraph, as described below.
  ...
  |`withConnectionRetry`  |No| Number of retries that the Nebula Java Client connected to the NebulaGraph. The default value is `1`.  |
...
- `WriteNebulaVertexConfig` is the configuration of the write vertex, as described below.
  ...
  |`withVidPolicy`  |No|  When writing the vertex ID, NebulaGraph 2.x use mapping function, supports HASH only. No mapping is performed by default.  |
...
- `WriteNebulaEdgeConfig` is the configuration of the write edge, as described below.
  ...
  |`withSrcPolicy`  |No| When writing the starting vertex ID, NebulaGraph 2.x use mapping function, supports HASH only. No mapping is performed by default.   |
  |`withDstIdField`  |Yes| The column in the DataFrame that serves as the destination vertex.   |
  |`withDstPolicy`  |No| When writing the destination vertex ID, NebulaGraph 2.x use mapping function, supports HASH only. No mapping is performed by default.   |
