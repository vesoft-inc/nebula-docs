...
After Exchange is compiled, copy the conf file `target/classes/application.conf` settings HBase data source configuration. In this case, the copied file is called `hbase_application.conf`. For details on each configuration item, see [Parameters in the configuration file](../parameter-reference/ex-ug-parameter.md).
...
Run the following command to import HBase data into Nebula Graph. For a description of the parameters, see [Options for import](../parameter-reference/ex-ug-para-import-command.md).
