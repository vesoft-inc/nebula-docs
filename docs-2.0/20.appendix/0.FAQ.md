...
### Can Chinese characters be used as identifiers, such as the names of graph spaces, tags, edge types, properties, and indexes?

No. The names of graph spaces, tags, edge types, properties, and indexes must use English letters, numbers, or underlines. Chinese characters are not currently supported.
...
There is no such command. You can use [Nebula Algorithm](../nebula-algorithm.md).
...
- If you install Nebula Graph by compiling the source code
  ...
  1. Use the `./<binary_name> --version` command to get the Git commit IDs of the Nebula Graph binary files. For example:
  ...
  3. Compare the commit time with the [Release time](https://github.com/vesoft-inc/nebula/releases)of Nebula Graph versions to find out the version of the Nebula Graph services.
