...
From Nebula Graph version 3.0, the Storage services added in the configuration files **CANNOT** be read or written directly. The configuration files only register the Storage services with the Meta service. You must run the `ADD HOSTS` command to read and write data to the Storage services. For more information, see [Manage logical rack](../4.deployment-and-installation/5.zone.md)。
