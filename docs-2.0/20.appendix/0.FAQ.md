...
Starting with Nebula Graph 3.0, the query statements `LOOKUP`, `GO`, and `FETCH` must specify output results with the `YIELD` clause. For more information, see [YIELD](../3.ngql-guide/8.clauses-and-options/yield.md).
