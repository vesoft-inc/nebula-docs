...
### Is there any message before and after the license expires, and how to renew the license after it expires?

Before your purchased license expires, the expiration reminders for different licenses are as follows:

- Nebula Graph Enterprise Edition license: 7 days before the license expires, there is an expiration reminder when Nebula Graph is started, and a 3-day buffer period after expiration. During the buffer period, you can continue using Nebula Graph. After the buffer period ends, Nebula Graph will be down and cannot be started.

- Dashboard/Explorer Enterprise Edition license: 14 days before the license expires. After the expiration, Dashboard/Explorer Enterprise Editions will be deactivated without a buffer period. 

!!! note

    The trial license of Dashboard/Explorer Enterprise Editions can be used for 15 days. 5 days before the license expires, there is an expiration reminder. After expiration, Dashboard/Explorer Enterprise Editions cannot be used. For a trial of Dashboard/Explorer Enterprise Editions, see [Online trial on our website](https://nebula-graph.com.cn/). The trial license of Nebula Graph Enterprise Edition has a variable trial period, so the reminder time before expiration is also variable.
