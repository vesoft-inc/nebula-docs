...
This is caused by the release of NebulaGraphConsole 2.6.0, not the change of NebulaGraph core. And it will not affect the content of the returned data itself.
...
You can use [NebulaGraphAlgorithm](../graph-computing/nebula-algorithm.md).
...
- You can also use NebulaGraphOperator to scale Graph and Storage services. For details, see [Deploy NebulaGraph clusters with Kubectl](../nebula-operator/3.deploy-nebula-graph-cluster/3.1create-cluster-with-kubectl.md) and [Deploy NebulaGraph clusters with Helm](../nebula-operator/3.deploy-nebula-graph-cluster/3.2create-cluster-with-helm.md).
