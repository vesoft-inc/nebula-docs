...
- **Backup and recovery**：NebulaGraph supports data backup and recovery. Users can use NebulaGraph Operator to backup the data of the NebulaGraph cluster to storage services, and can also restore data to the cluster from the storage service. For details, see [Backup and restore using NebulaGraph Operator](4.cluster-administration/4.6.backup-and-restore/4.6.0.overview.md).
...
[NebulaGraph Operator release notes](../20.appendix/release-notes/operator-release-note.md)

[History releases](https://github.com/vesoft-inc/nebula-operator/releases/tag/{{operator.tag}})
