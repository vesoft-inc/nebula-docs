...
- Run the following command to perform a full backup for the entire cluster whose meta service address is `127.0.0.1:9559`, and save the backup file to `/home/nebula/backup/`.
  ...
  !!! caution
        ...
        If you back up data to the local disk, only the data on the leader metad is backed up by default. So if there are multiple metad, you need to manually copy the directory of the leader metad (path `<storage_path>/meta`) and overwrite the corresponding directory of other follower meatd.
