...
For example, run the following command to scale up a Nebula Graph cluster by changing the number of Storage services from 2 (the original value) to 5:  
...
Similarly, you can scale down a Nebula Graph cluster by setting the value of the `replicas` corresponding to the different services in the cluster smaller than the original value.
