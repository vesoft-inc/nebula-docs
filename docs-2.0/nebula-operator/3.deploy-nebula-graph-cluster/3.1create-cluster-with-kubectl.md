  ...
  - For a NebulaGraph Enterprise cluster
    ...
    === "Cluster with Zones"
        ...
        ???+ note "Learn more about Zones in NebulaGraph Operator"
            ...
            Each Zone is a logical group of Storage pods and Graph pods and stores complete graph space data. NebulaGraph space data is partitioned and partition replicas are evenly distributed across all zones. Then, by assigning the Graph service for intra-Zone data access, you can reduce the cost of inter-Zone network traffic, and increase data transfer speed.

            To use the Zone feature, you must first know the actual zone information where your cluster nodes are located (nodes deployed on cloud platforms generally have zone node labels). After that, add the zone information by setting the `spec.metad.config.zone_list` parameter to a list of Zone names, split by comma, in the cluster configuration file. **Make sure that the Zone names are consistent with the actual zone names where the nodes are located.** For example, if the nodes are located in `az1`, `az2`, and `az3`, you can set the `spec.metad.config.zone_list` parameter to `az1,az2,az3`.
            ...
            NebulaGraph Operator uses the [TopoloySpread](https://kubernetes.io/docs/concepts/workloads/pods/pod-topology-spread-constraints/) feature of Kubernetes to control the scheduling of Storage or Graph pods. After `zone_list` is configured, the Storage services are added to a Zone after getting the Zone name by the `topology.kubernetes.io/zone` label.
            
            Whereas, to implement the intra-zone data access, the Graph service assigns itself to a Zone using `--assigned_zone=$NODE_ZONE` after recognizing the Zone name of the node where the Graph service is located by using an init-container to get the Zone name. NebulaGraph Operator uses `spec.alpineImage` to specify the Alpine Linux image used to obtain the Zone information where nodes are located. The default value is `reg.vesoft-inc.com/nebula-alpine:latest`. By adding the `spec.graphd.config.prioritize_intra_zone_reading` to `true` in the cluster configuration file, the Graph service prioritizes sending queries to the Storage services in the same Zone as the Graph service. If reading fails in that Zone, it decides based on `spec.graphd.config.stick_to_intra_zone_on_failure` whether to read the leader partition replica data across Zones. If `spec.graphd.config.stick_to_intra_zone_on_failure` is set to `true`, the Graph service does not read data from other Zones and returns an error. Otherwise, it reads the leader partition replica data from other Zones.
            ...
            After Storage and Graph services are assigned to Zones, the mapping between the Storage or Graph pod and the Zone is written to configmap named `nebula-graphd|storaged-zone`. Based on the mapping, pod scheduling back to the original Zones is supported during rolling updates and pod restarts.
        ...
        Part of the sample configuration:
