...
The following shows how to scale up a Nebula Graph cluster by changing the number of Storage services to 5:

1. Change the value of `storaged.replicas` in `apps_v1alpha1_nebulacluster.yaml` from `3` to `5`.
...
3. Check the number of Storage services.
  ...
  nebula-storaged-2   1/1     Running   0          2ms
...
The principle of scaling down a cluster is the same as scaling up a cluster. You scale down a cluster if the numeric value of `replicas` in `apps_v1alpha1_nebulacluster.yaml` is changed smaller than the current number. For more information, see the **Scale up clusters** section above.
...
[Connect to Nebula Graph database](../4.connect-to-nebula-graph-service.md)
