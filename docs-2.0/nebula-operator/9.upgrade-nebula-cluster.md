...
- Only support upgrading the Nebula Graph version from {{upgrade_from}} to {{upgrade_to}}.
...
The version of the Nebula Graph cluster to be upgraded in this topic is `{{upgrade_from}}`, and its YAML file name is `apps_v1alpha1_nebulacluster.yaml`.
...
2. Edit the `apps_v1alpha1_nebulacluster.yaml` file by changing the values of all the `version` parameters from {{upgrade_from}} to v{{nebula.release}}.
  ...
  spec:
    graphd:
      ...
      version: v{{nebula.release}} //Change the value from {{upgrade_from}} to v{{nebula.release}}.
      service:
        type: NodePort
