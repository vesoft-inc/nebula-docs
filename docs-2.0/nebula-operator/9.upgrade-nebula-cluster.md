...
- Only support upgrading the Nebula Graph version from 3.0.0. to 3.1.x.
...
The version of the Nebula Graph cluster to be upgraded in this topic is `3.0.0`, and its YAML file name is `apps_v1alpha1_nebulacluster.yaml`.
...
2. Edit the `apps_v1alpha1_nebulacluster.yaml` file by changing the values of all the `version` parameters from v3.0.0 to v{{nebula.release}}.
  ...
  spec:
    graphd:
      ...
      version: v{{nebula.release}} //Change the value from v3.0.0 to v{{nebula.release}}.
      service:
        type: NodePort
