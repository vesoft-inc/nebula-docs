...
| `prioritize_intra_zone_reading`   | `false`       | When set to `true` and a Zone is specified in `assigned_zone`, it will prioritize reading replica data within that Zone. If reading fails in that Zone, it will decide based on `stick_to_intra_zone_on_failure` whether to read data from other Zones.<br/>When set to `false`, it will read from the leader partition replicas. | No                                    |
| `stick_to_intra_zone_on_failure`  | `false`       | When set to `true`, if reading fails in the Zone specified in `assigned_zone`, it won't read data from other Zones.<br/>When set to `false`, it reads the leader partition replica across zones. | No                                    |
