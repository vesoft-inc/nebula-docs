...
| Name | Predefined value | Description |
| - | - | - |
| `enable_space_level_metrics` | `false` | Enable or disable space-level metrics. Such metric names include the name of the graph space that it monitors, for example, `query_latency_us{space=basketballplayer}.avg.3600`. You can view the supported metrics with the `curl` command. For more information, see [Query Nebula Graph metrics](../../6.monitor-and-metrics/1.query-performance-metrics.md). |
