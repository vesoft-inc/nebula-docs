...
|`enable_record_slow_query`|`true`|Whether to record slow queries. <br/> Only available in NebulaGraph Enterprise Edition.<br/>When set to `true`, if a query's execution time exceeds the duration defined by `slow_query_threshold_us`, NebulaGraph will log that query to a log file. <br/>Additionally, the slow query will be cached in memory within the `graphd` process until the number of cached slow query records reaches the limit defined by `slow_query_limit`. <br/>Cached slow query records can be retrieved through an HTTP interface.|  No|
