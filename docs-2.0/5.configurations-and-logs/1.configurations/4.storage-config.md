...
| `rocksdb_kv_separation_threshold`  | `100`    |   RocksDB key value separation threshold. Values at or above this threshold will be written to blob files during flush or compaction. Set the parameter based on service data distribution. Unit: bytes.   |
| `rocksdb_blob_compression`  | `lz4`    | Compression algorithm for blobs. Optional values are  `no`, `snappy`, `lz4`, `lz4hc`, `zlib`, `bzip2`, `zstd`.     |
| `rocksdb_enable_blob_garbage_collection`  | `true`    | Whether to garbage collect blobs during compaction.     |

## misc configurations

!!! caution

    The configuration `snapshot` in the following table is different from the snapshot in Nebula Graph. The `snapshot` here refers to the stock data on the leader when synchronizing Raft.
