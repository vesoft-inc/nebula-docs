...
!!! caution
    ...
    Some parameter values in the configuration file can be dynamically modified during runtime. We label these parameters as **support** runtime dynamic modification in this article. If the `local_config` value is set to `true`, the configuration will be restored to the initial configuration after the service is restarted. For more information, see [Modify configurations](1.configurations.md).
...
| `storage_cache_capacity`      | `0`    |  The size of memory reserved for Storage caches. The value must be slightly greater than the sum of `vertex_pool_capacity` and `negative_pool_capacity`. The configuration is measured in MB.|
