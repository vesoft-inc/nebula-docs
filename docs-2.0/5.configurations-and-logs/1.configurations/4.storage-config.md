...
| `enable_empty_key_pool`       | `false`| Whether or not to add empty key pool in the cache. Only valid when storage cache is enabled. The empty key indicates the queried key but does not exist.  |
| `empty_key_pool_capacity`     | `50`   | The size of empty key pool. The configuration is measured in MB. |
| `empty_key_item_ttl`          | `300`  | The TTL of items of empty key pool. The configuration is measured in seconds.  |
