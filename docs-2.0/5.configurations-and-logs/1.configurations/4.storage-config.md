...
| `port`                    | `9779`            | Specifies RPC daemon listening port. The external port for Storage Service is `9779`. The internal port is `port+1`, namely `9780`. Nebula Graph uses the internal port for multi-replica interactions. |
