...
|`disable_page_cache`              |`false`|Enables or disables the operating system's page cache for Nebula Graph. The parameter value is default to `false` and page cache is enabled. If the value is set to `true`, page cache is disabled and sufficient block cache space must be configured for Nebula Graph.|
