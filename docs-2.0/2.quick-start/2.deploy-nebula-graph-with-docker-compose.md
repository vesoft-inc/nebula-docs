...
* If you are deploying Nebula Graph as a non-root user, the docker group will grants you with Docker-related privileges. For a detailed instruction, see [Docker document: Manage Docker as a non-root user](https://docs.docker.com/engine/install/linux-postinstall/#manage-docker-as-a-non-root-user).
...
The data format has been modified in Jan 27, 2021 and is incompatible with the previous data. Run `docker-compose down -v` to delete all your local data.
