...
* [MATCH](../3.ngql-guide/7.general-query-statements/2.match.md)
  ...
  | Match variable-length paths                | `MATCH p=(v:player{name:"Tim Duncan"})-[e:follow*1..3]->(v2) RETURN v2 AS Friends` | `minHop`: Optional. It represents the minimum length of the path. `minHop`: must be a non-negative integer. The default value is 1.<br>`minHop` and `maxHop` are optional and default to 1 and infinity respectively. The data type of `e` is the list.|
