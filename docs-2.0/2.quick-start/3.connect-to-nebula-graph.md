...
Nebula Graph supports multiple types of clients, including a CLI client, a GUI client, and clients developed in popular programming languages. This topic provides an overview of Nebula Graph clients and basic instruction on how to use the native CLI client, nebula-console.
...
* The Nebula Graph services are running normally.
...
5. In the command line interface, change the working directory to the one that the nebula-console binary file is stored.
...
6. Run the following command to connect to Nebula Graph.
    ...
    * For Windows:
    ...
    | `-u/-user` | Sets the username of your Nebula Graph account. Before enabling authentication, you can use any string as the password. |
    | `-p/-password` | Sets the password of your Nebula Graph account. Before enabling authentication, you can use any string as the password. |
