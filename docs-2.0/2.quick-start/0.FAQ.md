...
2.1 Scale in graphd ： remove graphd from client's code. close this graphd process；

2.2 Scale out graphd： prepare graphd's binary and config files in the new host. Modify the config files and add all existing metad's adress. Then start the new graphd process.

3.1 Scale in storaged：（All spaces' replace number must be greater than 1），ref to [Balance remove command](../8.service-tuning/load-balance/#storage_1)。After the command finish, stop this storaged process；

3.2 Scale out storaged：（All spaces' replace number must be greater than 1）prepare storaged's binary and config files in the new host， Modify the config files and add all existing metad's adress. Then start the new storagedd process.
