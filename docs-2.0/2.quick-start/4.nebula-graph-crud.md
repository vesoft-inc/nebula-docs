...
* Create a graph space:
    ...
    > **NOTE:** After the creation of a graph space, wait 20 seconds before running the `USE` statement to use the graph space.
...
> **NOTE:** After the creation of a tag or an edge type, wait 20 seconds before inserting vertices or edges that has the tag or edge type.
...
### Why can't I insert vertices with a tag right after creating the tag?

It is usually because Nebula Graph has not finished synchronizing the creation of the tag to all related internal services. Nebula Graph synchronizes the following operations in the next heartbeat cycle.
...
* `CREATE EDGE INDEX`
 ...
 The default heartbeat interval is 10 seconds. To make sure the follow-up operations work as expected, wait 20 seconds after performing the preceding operations.
