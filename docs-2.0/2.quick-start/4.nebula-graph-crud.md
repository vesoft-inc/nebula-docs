...
You can use the statements [MATCH](../3ngql-guide/../3.ngql-guide/7.general-query-statements/2.match.md), GO (doc TODO), FETCH (doc TODO), and [LOOKUP](../3.ngql-guide/7.general-query-statements/5.lookup.md) to retrieve vertex and edge data.

* `MATCH` is the most commonly used, powerful, and flexible statement for graph data querying. It relies on native indexes and patterns to match data stored in Nebula Graph.
...
* Use `MATCH`: Find the information of all players.
...
* Use `GO`: Search for the players that the player with VID "player100" follows. Filter the players that the player with VID "player100" follows whose age is equal to or greater than 35. Rename the columns in the result with `Teammate` and `Age`.
