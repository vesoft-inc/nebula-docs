...
| Tag              | The type of a vertex. It defines a group of properties that describes the types of vertices. |
...
| - | - |
| `partition_num` | Specifies the number of partitions in each replica. The suggested number of partitions is 5 times the number of hard disks in the cluster. For example, if you have 3 hard disks in the cluster, we recommend that you set 15 partitions.           |
...
|`vid_type` | Specifies the data type of VIDs in a graph space. Available values are `FIXED_STRING(N)` and `INT64`. `N` represents the maximum length of the VIDs and it must be a positive integer. The default value is `FIXED_STRING(8)`. If you set a VID length greater than `N`, Nebula Graph will throw an error. To set the integer VID for vertices, set `vid_type` to `INT64`. |
...
!!! note
    ...
    In UTF-8 encoding, a Chinese character occupies 3 bytes. You should set an appropriate index length according to the variable-length property. For example, the index should be 30 bytes for 10 Chinese characters. For more information, see [CREATE INDEX](../3.ngql-guide/14.native-index-statements/1.create-native-index.md)
