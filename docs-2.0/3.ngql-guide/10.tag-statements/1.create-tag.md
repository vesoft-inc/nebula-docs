...
- `prop_name`
    ...
    `prop_name` is the name of property. It must be unique for each tag.
...
- `DEFAULT`
    ...
    Specifies a default value for a property. The default value can be any of the data type or expressions supported by Nebula Graph. If no value is specified, the default value is used when inserting a new vertex.
...
- `TTL_COL`
    ...
    The data type of the `prop_name` must be either `int` or `timestamp`.
