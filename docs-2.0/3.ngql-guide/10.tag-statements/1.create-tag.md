...
The `CREATE TAG` statement creates a tag with the given name in a graph space.
...
To run the `CREATE TAG` statement, you must have the [privilege](../../7.data-security/1.authentication/3.role-list.md) for the specified graph space. Otherwise, Nebula Graph throws an error.
...
|`COMMENT`|The remarks of a property or a vertex. The maximum length is 256 bytes. By default, there will be no comments on a tag.|
