...
- You must have the corresponding privilege to run the `DROP TAG` statement. For more information, see [Built-in role privileges](../../7.data-security/1.authentication/3.role-list.md).
...
- `tag_name`: Specifies the tag name that you want to drop. You can drop only one tag at a time.
