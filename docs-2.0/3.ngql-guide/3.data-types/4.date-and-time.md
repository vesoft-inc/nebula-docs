...
3. Query whether the property `p1` on the `test1` is `2021-03-17`.
...
5. Query the property `p3` on the tag `date1` less than the value of `2023-01-01T00:00:00.000000`.
...
5. Create a tag named `school` with the property of `TIMESTAMP`.

    ```ngql
    nebula> CREATE TAG IF NOT EXISTS school(name string , found_time timestamp);
    ```

6. Insert a vertex named `DUT` with a found-time timestamp of `"1988-03-01T08:00:00"`.
