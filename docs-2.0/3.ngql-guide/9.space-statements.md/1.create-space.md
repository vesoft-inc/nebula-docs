...
CREATE SPACE [IF NOT EXISTS] <graph_space_name>
    ...
    vid_type = {FIXED_STRING(<max_string_length>))] | INT64}
```
...
* `vid_type`
    ...
    Specifies the data type of vertex IDs (VIDs) in a graph space. The default value is `FIXED_STRING(8)`. This shows that the default VID data type is fixed `string` and the default length of `8`. In Nebula Graph 2.0.0-RC, only `FIXED_STRING(N)` and `INT64` are supported. `N` represents the maximum length of the VIDs. You must set the `fixed_string` to a positive integer. If your VID length is greater than the maximum VID length, Nebula Graph throws an error. To set the integer VID for vertices, specify the `vid_type` to `INT64`.
