...
- Make sure that the added attribute name is not the same as an existing or deleted attribute name, otherwise the added attribute will fail.

- Allows to increase the length of `FIXED_STRING` and `INT`.
