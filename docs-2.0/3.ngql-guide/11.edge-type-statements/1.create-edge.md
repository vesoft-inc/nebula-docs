...
|`<edge_type_name>`|The edge type name must be **unique** in a graph space. Once the edge type name is set, it can not be altered. The name of the edge type is case-sensitive and allows Non-English, letters, numbers, or underlines. When Non-English characters are included or using reserved keywords only in the name, quote them with backticks. For more information, see [Keywords and reserved words](../../3.ngql-guide/1.nGQL-overview/keywords-and-reserved-words.md).|
