...
The `DATE` type is used for values with a date part but no time part. Nebula Graph retrieves and displays `DATE` values in `YYYY-MM-DD` format. The supported range is `-32768-01-01` to `32767-12-31`.
