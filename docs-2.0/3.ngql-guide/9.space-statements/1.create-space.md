...
|`zone_list`|Specify the list of Zones to which the graph space belongs. Partition replicas will be distributed across these Zones. The number of replicas cannot exceed the number of specified Zones. When `zone_list` is not specified, partition replicas are distributed across all Zones by default. For more information, see [Manage zones](../../4.deployment-and-installation/5.zone.md).|
