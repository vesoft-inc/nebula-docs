...
|`<index_name>`|The name of the index. It must be unique in a graph space. A recommended way of naming is `i_tagName_propName`. The name of the index is case-sensitive and allows Chinese, letters, numbers, or underlines. When Chinese characters are included or using reserved keywords only in the name, quote them with backticks. For more information, see [Keywords and reserved words](../../3.ngql-guide/1.nGQL-overview/keywords-and-reserved-words.md).|
