...
The `UPSERT` statement is a combination of `UPDATE` and `INSERT`. You can use `UPSERT VERTEX` to update properties of a vertex if it exists or insert a new vertex if it does not exist.
...
```ngql
// This query checks if the following three vertices exists. The result "Empty set" indicates that the vertices do not exist.
