...
A `MATCH` statement defines a [search pattern](../1.nGQL-overview/3.graph-patterns.md) and uses it to match data stored in NebulaGraph and to retrieve them in the form defined in the `RETURN` clause.
...
!!! Performance
    ...
    In NebulaGraph {{ nebula.release }}, the `MATCH` statement has initially optimized for resource usage and performance.
