...
Make sure there is at least one index for the `MATCH` statement to use. If you want to create an index on a property, but there are already vertices or edges with that property, rebuild the index after creation to make it take effect on existing data.
...
You can use the `DISTINCT` keyword to remove duplicate results.
