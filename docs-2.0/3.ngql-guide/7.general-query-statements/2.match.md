...
- `pattern`: For detailed description of pattern, see [Patterns](../1.nGQL-overview/3.graph-patterns.md). The `MATCH` statement supports matching one or more patterns, separated by commas. For example: `(a)-[]->(b),(c)-[]->(d)`.

- `clause_1`: The `WHERE`, `WITH`, `UNWIND`, `OPTIONAL MATCH` clauses are supported, and the `MATCH` clause can also be used.

- `output`: Define the output to be returned, you can alias the output using `AS`.
...
- `clause_2`: The `ORDER BY`, `LIMIT` clauses are supported.
...
To match vertex with multiple tags, use colons.
...
If you need to determine the target vertex, you can use the `CASE` expression.
