...
> **nGQL compatibility**
>
> * In nGQL 1.x, the `--` symbol is used for adding inline comments.
> * Starting from nGQL 2.0, the `--` symbol represents an undirected edge.
...
And you can add a `>` or `<` symbol to make the edge directed. nGQL supports both incoming and outgoing edges.
...
```nGQL
(nebula@nebula) [nba]> MATCH (v:player{name:"Tim Duncan"}) -[e]- (v2) RETURN e;
+---------------------------------------------------------------------------+
| e                                                                         |
+---------------------------------------------------------------------------+
...
```nGQL
(nebula@nebula) [nba]> MATCH (v:player{name:"Tim Duncan"}) -[e:serve]- (v2) RETURN e;
+---------------------------------------------------------------------------+
| e                                                                         |
+---------------------------------------------------------------------------+
...
```nGQL
(nebula@nebula) [nba]> MATCH (v:player{name:"Tim Duncan"}) -[e:like{likeness:95}]-> (v2) RETURN e;
+------------------------------------------------------+
| e                                                    |
+------------------------------------------------------+
...
```nGQL
(nebula@nebula) [nba]> MATCH (v:player{name:"Tim Duncan"}) -[e:like|:serve]-> (v2) RETURN e;
+---------------------------------------------------------------------------+
| e                                                                         |
+---------------------------------------------------------------------------+
| ("player100")-[:like@0{likeness: 95}]->("player101")                      |
...
### Match variable-length edges

TODO

## Retrieve data with MATCH

This section summarizes the items you can retrieve with `RETURN` in a MATCH `statement`.

### Retrieve vertex information

Use `RETURN <vertex_name>` to retrieve the VID, tag and properties of a vertex.

```nGQL
(nebula@nebula) [nba]> MATCH (v:player{name:"Tim Duncan"}) RETURN v;
+----------------------------------------------------+
| v                                                  |
+----------------------------------------------------+
| ("player100" :player{name: "Tim Duncan", age: 42}) |
...
Use the `id()` function to retrieve the VID of a vertex.

```nGQL
(nebula@nebula) [nba]> MATCH (v:player{name:"Tim Duncan"}) RETURN id(v);
+-------------+
| id(v)       |
+-------------+
| "player100" |
...
Use the `labels()` function to retrieve the tag on a vertex.

```nGQL
(nebula@nebula) [nba]> MATCH (v:player{name:"Tim Duncan"}) RETURN labels(v);
+------------+
| labels(v)  |
+------------+
| ["player"] |
...
The information returned by the `labels()` function is a list. The following example shows how to retrieve the first tag in the list.

```nGQL
(nebula@nebula) [nba]> MATCH (v:player{name:"Tim Duncan"}) RETURN labels(v)[0];
+--------------+
| labels(v)[0] |
+--------------+
| "player"     |
...
If the specific position in the list has no tag, `NULL` is returned.

```nGQL
(nebula@nebula) [nba]> MATCH (v:player{name:"Tim Duncan"}) RETURN labels(v)[1];
+--------------+
| labels(v)[1] |
+--------------+
| NULL         |
+--------------+
Got 1 rows (time spent 2066/2754 us)
```

### Retrieve vertex properties

Use `RETURN <vertex_name>.<property>` to retrieve a vertex property.

```nGQL
(nebula@nebula) [nba]> MATCH (v:player{name:"Tim Duncan"}) RETURN v.age as Age;
+-------+
| v.age |
+-------+
| 42    |
...
Use `AS` to specify an alias for the vertex property.
...
Use `RETURN <edge_name>` to retrieve the information of an edge.
