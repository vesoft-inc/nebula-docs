...
When no [index](../14.native-index-statements/1.create-native-index.md) has been created, the `MATCH` statements is only supported in the following cases. When you get an error executing the `MATCH` statement, you can create and rebuild the index and then execute the `MATCH` statement.
