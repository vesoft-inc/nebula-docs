...
```ngql
// Returns player 100.
nebula> GO FROM "player100" OVER follow REVERSELY YIELD follow._dst;
+-------------+
| follow._src |
+-------------+
| "player100" |
...
nebula> MATCH (v)<-[e:follow]- (v2) WHERE id(v) == 'player100' RETURN id(v2)
+-------------+
| id(v2)      |
+-------------+
| "player101" |
...
// This MATCH query shares the same semantics with the preceding GO query.
nebula> MATCH (v)<-[e:follow]- (v2)-[e2:serve]->(v3)  WHERE id(v) == 'player100' RETURN v2.name, v3.name;
+---------------------+-----------------+
| v2.name             | v3.name         |
+---------------------+-----------------+
| "Tony Parker"       | "Spurs"         |
...
// This MATCH query shares the same semantics with the preceding GO query.
nebula> MATCH (v) -[e:follow]-(v2) WHERE id(v)== "player102" RETURN id(v2);
+-------------+
| id(v2)      |
+-------------+
| "player100" |
...
// This MATCH query shares the same semantics with the preceding GO query.
nebula>  MATCH (v) -[e:follow*1..2]->(v2) WHERE id(v) == "player100" RETURN id(v2);
+-------------+
| id(v2)      |
+-------------+
| "player101" |
