...
- `YIELD [DISTINCT] <return_list>`: defines the output to be returned. It is recommended to use the [Schema function](../6.functions-and-expressions/4.schema.md) to fill in `<return_list>`. `src(edge)`, `dst(edge)`, `type(edge) )`, `rank(edge)`, `properties(edge)`, `id(vertex)`, and `properties(vertex)` are currently supported, while nested functions are not. For more information, see [YIELD](../8.clauses-and-options/yield.md). When not specified, the destination vertex IDs will be returned by default.
