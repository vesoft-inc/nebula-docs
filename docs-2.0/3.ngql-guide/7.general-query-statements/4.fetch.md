...
|`vid`|The vertex ID.|
|`YIELD`|Define the output to be returned. It returns the defined properties and `VertexID`. For details, see [`YIELD`](../8.clauses-and-options/yield.md). If there is no `YIELD` clause, `vertices_` is returned by default, which contains all the information about the vertex. |
|`AS`| Set an alias.|
...
|`YIELD`|Define the output to be returned. It returns the defined properties, `SrcVertexID`, `DstVertexID`, and `rank`. For details, see [`YIELD`](../8.clauses-and-options/yield.md). If there is no `YIELD` clause, `edges_` is returned by default, which contains all the information about the edge. |
