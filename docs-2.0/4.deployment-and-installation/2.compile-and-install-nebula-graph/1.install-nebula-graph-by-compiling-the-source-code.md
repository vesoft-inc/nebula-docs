...
1. Use Git to clone the source code of Nebula Graph to your host.
   ...
   - To install a specific version of Nebula Graph 2.x, use the `--branch` option to specify the correct branch. For example, to install 2.0.0, run the following command.
...
5. Compile Nebula Graph.
    ...
    To speed up the compiling, use the `-j` option to set a concurrent number `N`. Devide the memory size in GB by 2, and compare the result with the CPU core number. Set the smaller number between them as the concurrent number `N`.
