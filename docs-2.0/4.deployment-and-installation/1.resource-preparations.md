...
!!! enterpriseonly
     ...
     Nebula Graph {{ nebula.release }} cannot be run or compiled directly on the ARM architecture (including Apple Mac M1 or Huawei Kunpeng), but you can run containerized Nebula Graph databases on Docker Desktop for ARM macOS or on ARM Linux servers. 
