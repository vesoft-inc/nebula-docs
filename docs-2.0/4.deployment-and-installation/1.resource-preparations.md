...
!!! enterpriseonly
    ...
    In addition to installing Nebula Graph with the source code, the Dashboard Enterprise Edition tool is a better and convenient choice for installing Community and Enterprise Edition Nebula Graph. For details, see [Deploy Dashboard](../nebula-dashboard-ent/2.deploy-connect-dashboard-ent.md).
...
- Use local SSD devices, or AWS Provisioned IOPS SSD equvalance.
...
|:--- |:---|:--- |:---|
| Disk space for a cluster               |Bytes| `the_sum_of_edge_number_and_vertex_number` * `average_bytes_of_properties` * 6 * 120%      |For more information, see [Edge partitioning and storage amplification](../1.introduction/3.nebula-graph-architecture/4.storage-service.md).|
