...
The correspondence between the Nebula Algorithm release and the NebulaGraph core release is as follows.

|Algorithm client|NebulaGraph|
|:---|:---|
...
- The NebulaGraph services have been deployed and started. For details, see [Nebula Installation](4.deployment-and-installation/1.resource-preparations.md).
...
- When writing the algorithm results into the NebulaGraph, make sure that the Tag in the corresponding graph space has properties corresponding to the algorithm result name. The corresponding properties of each algorithm are as follows.
...
1. Read the graph data of DataFrame from the NebulaGraph database using the Nebula Spark Connector.
