...
- The storage structure of vertices
  ...
  Different from NebulaGraph version 2.x, version 3.x added a new key for each vertex. Compared to the old key that still exists, the new key has no `TagID` field and no value after enabling the configuration without tag. Vertices in NebulaGraph can now live without tags owing to the new key.
