...
!!! note
    ...
    Space-level metrics can be queried only by querying all metrics, for example, `curl -G "http://192.168.8.40:19559/stats"`. The return result contains the graph space name in the form of '{space=space_name}', for example, `num_active_queries{space=basketballplayer}.sum.5=0`.
