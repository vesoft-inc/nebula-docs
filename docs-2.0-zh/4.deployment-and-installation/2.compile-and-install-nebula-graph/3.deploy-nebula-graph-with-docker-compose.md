...
在小概率的情况下，Docker Compose 中的 Storaged 激活脚本并没有在适当的时机得到执行。如果发现 Storaged 容器的健康状态一直是`offline`，可以通过 [NebulaGraph Console](../../nebula-console.md) 或者 [NebulaGraph Studio](../../nebula-studio/deploy-connect/st-ug-connect.md) 连接 NebulaGraph，并手动执行`ADD HOSTS`命令对其进行激活。激活查询示例如下：
