...
# 查询所有name等于Tony Parker的点数据，返回name和age属性值
nebula> LOOKUP ON player \
        WHERE player.name == "Tony Parker" \
...
# 查询所有name以B开头，且age在22和30之间的点数据，并返回name和age属性值
nebula> LOOKUP ON player \
...
# 查询所有name等于Kobe Bryant的点数据，返回点的ID和name属性值，并以此为起始点进行遍历，返回遍历结果
nebula> LOOKUP ON player \
...
# 根据degree属性值升序排列，并返回前10条边的degree属性值
nebula> LOOKUP ON follow \
...
# 查询所有degree等于60的边，返回边的目的点ID和边的degree属性值，并为此起始点进行遍历，返回遍历结果
nebula> LOOKUP ON follow \
...
- 查找 Edge type 为`follow`的所有边的信息。
    ...
    ```ngql
    # 创建名为follow的Edge type
    nebula> CREATE EDGE IF NOT EXISTS follow(degree int);
    
    # 创建Edge type为follow的索引follow_index
