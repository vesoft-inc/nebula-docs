...
### nebula-importer 导入数据输出的字段都是什么意思？
...
在使用 nebula-importer 导入数据时，会遇到上面的结果输出。以下是字段说明：

* `Records` 对应的是 csv 的记录数（依次分别是）：完成的条数、失败的条数、每秒导入多少条数据；
* `Requests` 对应的是请求数（依次分别是）：完成的请求数、失败的请求数、请求耗时：服务端 / 客户端，每秒处理的请求数；
* `Processed` 对应的是处理的点边数（依次分别是）：完成的处理点边总数、失败的点边数量、每秒处理的点边数量；
