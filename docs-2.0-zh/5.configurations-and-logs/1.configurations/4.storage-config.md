...
!!! note
    ...
    Memory Tracker 是一个内存管理工具，用于监控和限制内存使用。对于大型查询，Memory Tracker 可以防止内存泄漏和 OOM 问题。如果在容器环境中使用 Memory Tracker，需要在 Storage 和 Graph 服务的配置文件中添加相关配置。操作如下：

    1. 创建 `/sys/fs/cgroup/graphd/`，`/sys/fs/cgroup/storaged/`，并配置各自目录下的`memory.max`。
    2. 在`etc/nebula-graphd.conf`，`etc/nebula-storaged.conf`添加相关配置:
